...
export default function TradingTerminal() {
  ...
  const load = useCallback(async (s: string) => {
    ...
    try {
      ...
      if (!ltfRes.ok || !htfRes.ok) throw new Error("Binance market data could not be loaded.");
      ...
      setError(e instanceof Error ? e.message : "Unable to load market data."); setStatus("offline");
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`wss://data-stream.binance.vision/ws/${symbol.toLowerCase()}@kline_15m`);
    wsRef.current = ws;
  ...
  return <main className="terminal-shell">
    <header className="topbar">
      ...
      <div className="market-picker"><label htmlFor="symbol">Market</label><select id="symbol" value={symbol} onChange={e => setSymbol(e.target.value)}>{SYMBOLS.map(s => <option key={s}>{s}</option>)}</select></div>
      <div className="connection"><span className={`status-dot ${status}`} />{status === "live" ? "BINANCE LIVE" : status === "connecting" ? "CONNECTING" : "OFFLINE"}</div>
    </header>
    <section className="market-strip"><div><div className="eyebrow">BINANCE SPOT</div><h1>{symbol}</h1></div><div className="quote-block"><div className="price">{price == null ? "—" : formatPrice(price)}</div><div className="quote-note">15m real-time candle stream</div></div><div className="tf-stack"><span className="tf active">15m</span><span className="tf">HTF 4h</span></div></section>
    <section className="workspace">
      <div className="chart-card"><div className="chart-toolbar"><div><span className="toolbar-title">Price action</span><span className="toolbar-muted">15 minute execution chart</span></div><div className="toolbar-badges"><span className="badge">HTF: 4H</span><span className="badge">LTF: 15M</span><span className="badge live-badge">LIVE</span></div></div><div className="chart-wrap">{loading && <div className="chart-overlay">Loading Binance candles…</div>}<div ref={rootRef} className="chart" /></div></div>
    ...
    <footer className="footer"><span>Public Binance market data · no API key required for this market-data-only build.</span><span>Educational / backtesting software — not financial advice.</span></footer>
  </main>;
}
